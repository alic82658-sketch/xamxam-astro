...
for (const required of ['/','/actualites/','/guides/','/metiers/','/outils/','/afrique/','/a-propos/','/contact/','/guides/cv-adapte-offre/','/guides/preparer-cours-avec-ia/','/guides/premiers-usages-ia-pme/','/blog/supprimer-fichiers-chatgpt-library/','/ecosysteme/','/ecosysteme/andakia-awa/']) assert(exists(required), `Page absente : ${required}`);
let links=0;
...
for (const label of ['Commerçants','Commerciaux','Professeurs','Dirigeants de PME','Immobilier','Emploi']) assert(home.includes(label),`Parcours absent de l’accueil : ${label}`);
assert((await stat(join(root,'images/editorial/gemini-etudes-bureau-v2.webp'))).size>1000);
...
console.log(`${htmlFiles.length} pages contrôlées ; ${links} liens et visuels internes valides ; six parcours métiers ; canoniques, données structurées et redirections vérifiés.`);
